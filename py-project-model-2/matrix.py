...
class Matrix:
    def __init__(self, previous_u, a):
        ...
        self.a = a
        self.delta_t = Input.DELTA_T
        self.delta_x = Input.DELTA_X
        self.n = int(Input.LENGTH / Input.DELTA_X)
    ...
    def generate_column(self):
        ...
        first = self.previous_u[1] - self.coefficient() * \
            (self.previous_u[2] - 2 * self.previous_u[1] + self.previous_u[0] + Input.U_0)
        column.append(first)
        ...
        last = self.previous_u[self.n - 1] - self.coefficient() * \
            (self.previous_u[self.n] - 2 * self.previous_u[self.n - 1] + self.previous_u[self.n - 2] + Input.U_LENGTH)
        column.append(last)
